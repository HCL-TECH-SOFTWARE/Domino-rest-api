...
The client ID described here applies only to the Domino REST API server. It is not used for the Admin UI or for Office Forms Based Authentication (OFBA) used in attachment editing. To support these features, configure at least the following clients in your identity provider:
